...
const LOCALES = {
  zh: {
    ...
    'tool.videoWarn': '视频转码在浏览器本地实时进行，不依赖服务器、无需下载引擎；转码耗时约等于视频时长，请保持页面在前台',
  ...
  en: {
    ...
    'tool.videoWarn': 'Transcoding runs locally in your browser in real time — no server, no engine download. It takes about as long as the video itself, so keep this tab in the foreground.',
  ...
  ja: {
    ...
    'tool.videoWarn': '変換はブラウザ内でリアルタイムに完結しサーバー不要・エンジンのダウンロードも不要。所要時間は動画の長さとほぼ同じなので、タブを前面に保ってください',
