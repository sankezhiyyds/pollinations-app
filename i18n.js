...
const LOCALES = {
  zh: {
    ...
    'usage.modeKey': '模式：API Key',
    'usage.modeAnon': '模式：匿名',
    'usage.balance': '余额：{c} 积分',
    'usage.balanceUnknown': '余额：不可读取',
    'usage.total': '本次总消耗：≈ {c} 积分',
    'usage.details': '文本 {tokens} tokens · 图片 {images} 次 · 音频 {audio} 次 · 视频 {video} 次',
    'usage.modelSynced': '模型同步 {t}',
    'billing.free': '免费',
    'billing.paid': '按量付费',
    'billing.keyRequired': '按量付费，需 API Key',
    'billing.serverPrice': '价格由服务端决定',
    'billing.anonSize': '匿名可用尺寸',
    'billing.keySize': '需 API Key 的扩展尺寸',
    'billing.anonResolution': '匿名可用分辨率',
    'billing.paidParameter': '影响最终费用',
    ...
    'cost.image': '计费说明：flux、sana 标记为免费；其余已知模型按量付费。未知模型显示“价格由服务端决定”。尺寸只表示匿名可用范围或 Key 权限，不代表模型免费。',
    'cost.audio': '计费说明：grok-tts、qwen-tts 标记为免费；其余已知模型按量付费。实际金额以服务端余额变化为准。',
    'cost.video': '计费说明：模型先标明免费或按量付费；分辨率、时长和配音都会影响最终费用。实际金额以服务端余额变化为准。',
    'cost.text': '计费说明：openai、openai-fast 标记为免费；其余已知模型按 token 计费。未知模型价格由服务端决定。',
    ...
    'txt.modelNeedKey': '按量付费：需要 API Key',
    'txt.modelFree': '免费：无需积分；文本按 token 统计',
    'txt.modelPremium': '按量付费：按 token 计费',
    'aud.modelNeedKey': '按量付费：需要 API Key',
    'aud.modelFree': '免费：无需积分；仍以服务端账单为准',
    'aud.modelPremium': '按量付费：金额随模型和参数变化',
    'vid.modelNeedKey': '按量付费：需要 API Key',
    'vid.modelFree': '免费：无需积分；分辨率和时长仍受档位限制',
    'vid.modelPremium': '按量付费：分辨率、时长和配音会影响费用',
    ...
    'tool.qMedium': '中（默认）',
    'tool.qLow': '低（文件更小）',
    'tool.webpFormat': 'WEBP（动图）',
    'tool.gifFormat': 'GIF（动图）',
    'tool.videoWarn': '视频转码在浏览器本地逐帧进行，不依赖服务器、无需下载引擎；动态 WebP 和 GIF 可能需要更长时间，请保持页面在前台',
    'tool.formatUnsupported': '当前浏览器不支持该输出格式，请换用 WebM 或改用 Chrome/Edge',
    'tool.loadingEngine': '正在准备…',
    ...
    'ast.tokenCount': '本次消耗 {n} tokens ≈ {c} 积分（会话累计 {t} tokens ≈ {d} 积分）',
    'ast.creditSpent': '本次消耗约 {c} 积分',
  ...
  en: {
    ...
    'usage.modeKey': 'Mode: API key',
    'usage.modeAnon': 'Mode: anonymous',
    'usage.balance': 'Balance: {c} credits',
    'usage.balanceUnknown': 'Balance: unavailable',
    'usage.total': 'Total this session: ≈ {c} credits',
    'usage.details': 'Text {tokens} tokens · Images {images} · Audio {audio} · Video {video}',
    'usage.modelSynced': 'Models synced {t}',
    'billing.free': 'Free',
    'billing.paid': 'Pay per use',
    'billing.keyRequired': 'Pay per use, API key required',
    'billing.serverPrice': 'Price set by server',
    'billing.anonSize': 'Anonymous size',
    'billing.keySize': 'Extended size, API key required',
    'billing.anonResolution': 'Anonymous resolution',
    'billing.paidParameter': 'Affects final price',
    ...
    'cost.image': 'Billing: flux and sana are marked free; other known models are pay per use. Unknown models show “Price set by server”. Size indicates anonymous access or key requirements, not whether a model is free.',
    'cost.audio': 'Billing: grok-tts and qwen-tts are marked free; other known models are pay per use. Actual cost follows the server balance change.',
    'cost.video': 'Billing: each model is marked free or pay per use. Resolution, duration, and audio affect the final price. Actual cost follows the server balance change.',
    'cost.text': 'Billing: openai and openai-fast are marked free; other known models are billed by tokens. Unknown model prices are set by the server.',
    ...
    'vid.retryWait': 'Rate limited, retry in {n}s',
    ...
    'img.model': 'Model',
    'img.modelHint': 'Text labels before model names show billing status; actual cost follows the server bill',
    'img.modelFree': 'Free: no credits expected; check the server bill',
    'img.modelPremium': 'Pay per use: API key required; cost varies by model and parameters',
    'img.modelNeedKey': 'Pay per use: API key required',
    ...
    'tool.webpFormat': 'WEBP (animated)',
    'tool.gifFormat': 'GIF (animated)',
    'tool.videoWarn': 'Transcoding runs locally in your browser frame by frame — no server, no engine download. Animated WebP and GIF may take longer, so keep this tab in the foreground.',
    'tool.formatUnsupported': 'This browser can\u2019t encode the selected format. Try WebM, or use Chrome/Edge.',
    'tool.loadingEngine': 'Preparing\u2026',
    ...
    'ast.tokenCount': 'This request used {n} tokens ≈ {c} credits (session total {t} tokens ≈ {d} credits)',
    'ast.creditSpent': 'Spent ≈ {c} credits on this generation',
  ...
  ja: {
    ...
    'usage.modeKey': 'モード：APIキー',
    'usage.modeAnon': 'モード：匿名',
    'usage.balance': '残高：{c}クレジット',
    'usage.balanceUnknown': '残高：取得不可',
    'usage.total': '今回の合計消費：≈ {c}クレジット',
    'usage.details': 'テキスト {tokens}トークン · 画像 {images}回 · 音声 {audio}回 · 動画 {video}回',
    'usage.modelSynced': 'モデル同期済み {t}',
    'billing.free': '無料',
    'billing.paid': '従量課金',
    'billing.keyRequired': '従量課金、APIキー必須',
    'billing.serverPrice': '価格はサーバー側で決定',
    'billing.anonSize': '匿名で利用可能なサイズ',
    'billing.keySize': '拡張サイズ、APIキー必須',
    'billing.anonResolution': '匿名で利用可能な解像度',
    'billing.paidParameter': '最終料金に影響',
    ...
    'cost.image': '課金：flux と sana は無料表示、その他の既知モデルは従量課金です。不明なモデルは「価格はサーバー側で決定」と表示します。サイズは匿名利用範囲またはキー要件を示し、モデルの無料・有料とは別です。',
    'cost.audio': '課金：grok-tts と qwen-tts は無料表示、その他の既知モデルは従量課金です。実際の金額はサーバー残高の変化に従います。',
    'cost.video': '課金：モデルごとに無料または従量課金を表示します。解像度、長さ、音声は最終料金に影響します。実際の金額はサーバー残高の変化に従います。',
    'cost.text': '課金：openai と openai-fast は無料表示、その他の既知モデルはトークン課金です。不明なモデルの価格はサーバー側で決定されます。',
    ...
    'img.model': 'モデル',
    'img.modelHint': 'モデル名の前の文字ラベルが課金状態を示します。実際の金額はサーバー請求に従います',
    'img.modelFree': '無料：クレジット不要の想定。サーバー請求を確認してください',
    'img.modelPremium': '従量課金：APIキー必須。モデルとパラメータで料金が変わります',
    'img.modelNeedKey': '従量課金：APIキー必須',
    ...
    'txt.modelNeedKey': '従量課金：APIキー必須',
    'txt.modelFree': '無料：クレジット不要。テキストトークンは集計します',
    'txt.modelPremium': '従量課金：トークン単位',
    'aud.modelNeedKey': '従量課金：APIキー必須',
    'aud.modelFree': '無料：クレジット不要の想定。サーバー請求を確認してください',
    'aud.modelPremium': '従量課金：モデルとパラメータで料金が変わります',
    'vid.modelNeedKey': '従量課金：APIキー必須',
    'vid.modelFree': '無料：クレジット不要。解像度と長さは無料枠の制限内です',
    'vid.modelPremium': '従量課金：解像度、長さ、音声が料金に影響します',
    ...
    'tool.qLow': '低（ファイル小）',
    'tool.webpFormat': 'WEBP（アニメーション）',
    'tool.gifFormat': 'GIF（アニメーション）',
    'tool.videoWarn': '変換はブラウザ内でフレームごとに行われ、サーバー不要・エンジンのダウンロードも不要です。動的 WebP と GIF は時間がかかる場合があるため、タブを前面に保ってください',
    'tool.formatUnsupported': 'このブラウザは選択した形式に対応していません。WebM を選ぶか Chrome/Edge をご利用ください',
    'tool.loadingEngine': '準備中…',
    ...
    'ast.tokenCount': '今回は {n}トークン消費（約 {c}クレジット）。セッション合計 {t}トークン ≈ {d}クレジット',
    'ast.creditSpent': '今回は約 {c}クレジット消費したよ',
